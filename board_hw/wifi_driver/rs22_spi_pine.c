  ...
  do
  {
    ...
    switch(i)
    {
    	case 0:
    	{
    		...
    		rs22_send_TA_mgmt_frame((uint8_t *)&buffer,
    									RSI_DESC_LEN );
    	}
    	break;
  ...
  )
{
  ...
  status = rs22_card_write(&frame[16],
                             (len - 16),
                             SND_MGMT_Q);
  ...
  )
{
  ...
  *queueno = frame_desc[14] & RSI_DESC_QUEUE_NUM_MASK;
  ...
  else if(*queueno == RCV_TA_MGMT_Q)
  {
	 ...
	 te_len = te_len & 0xFF;
    if (te_len != 0)
    {/* Length is present and data is present  */
	
      length1 = CPU_TO_LE16(*(uint16_t *)&frame_desc[8]);
    }
    else
    {/* Length is 0 and data is not present */
  ...
  else
  {
    ...
    if(frame_desc[15] & RSI_DESC_AGGR_ENAB_MASK)
    {
      ...
      {
    	cur_length = CPU_TO_LE16((*(uint16_t *)&frame_desc[ii * 2]) & 0xFFF);
...
uint16_t rsi_send_join_request (struct WiFi_CNFG_Join_t *join_params)
{
  ...
  rs22_send_TA_mgmt_frame((uint8_t *)&mgmt,
                		  sizeof(struct RSI_Mac_frame_t) + RSI_DESC_LEN);
...
*END****************************************************************************/
int32_t rsi_send_TCP_config_request(uint8_t uUseDHCP,
									uint8_t uIPaddr[],
									uint8_t uNetMask[],
									uint8_t uDefaultGateway[])
{
  ...
  cmd.descriptor[0] = CPU_TO_LE16(sizeof(struct TCP_CNFG_Configure) - RSI_DESC_LEN);
#if NEW_FIRMWARE
...
*END****************************************************************************/
RS22_STATUS rsi_socket_create( SOCKET_TYPE uSocketType,
...
#if NEW_FIRMWARE
  cmd.descriptor[0] = CPU_TO_LE16(sizeof(struct TCP_CNFG_Socket) /*+ 56*/ - RSI_DESC_LEN);
  cmd.descriptor[3] = 0x5555;
#else
...
uint16_t rsi_socket_send(uint16_t uSocketDescriptor,
						  ...
						  uint8_t *pBuffer)
{
  ...
  *((uint16_t *)&cmd.descriptor[0]) = (uBufferLength + uDataOffset);
  ...
  status = rs22_send_data_frame((uint8_t *)&cmd,
                    			uBufferLength + uDataOffset + RSI_DESC_LEN);
...
#if NEW_FIRMWARE
  cmd.descriptor[0] = CPU_TO_LE16(sizeof(struct RSSI_QUERY_Req) /*+ 56*/ - RSI_DESC_LEN);
  cmd.descriptor[3] = 0x5555;
#else
...
*END****************************************************************************/
uint16_t
rsi_Send_Wlan_Conn_Status_Req ( void )
{
...
#if NEW_FIRMWARE
  cmd.descriptor[0] = CPU_TO_LE16(sizeof(struct WLAN_CONN_Query_Req) /*+ 56*/ - RSI_DESC_LEN);
  cmd.descriptor[3] = 0x5555;
#else
...
uint16_t rs22_socket_close(uint16_t uSocketDescriptor)
{
  ...
  rs22_memset(cmd.descriptor, 0, 9);
  cmd.descriptor[0] = CPU_TO_LE16(sizeof(struct TCP_CNFG_Close) - RSI_DESC_LEN);
  cmd.descriptor[3] = 0x5555;
...
int8_t rsi_send_ssp_init_cmd( void )
{
  struct bus_cmd_s  cmd;
  int32_t            status = RSI_STATUS_SUCCESS;
  uint32_t            ii = 0;
...
*END****************************************************************************/
uint32_t
rs22_send_ssp_mode_set
  (
    IN uint8_t         byte //1 - For High Speed Mode
  )
{
  struct bus_cmd_s    cmd;
  int32_t              status = RSI_STATUS_SUCCESS;
