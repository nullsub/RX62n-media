        ...
        if(Adaptor.tcp_configured == TRUE)
        {
          ...
          switch(subtype)
          {
			 case E_TCP_EVT_SocketCreateEvent:  /* Socket create event */
             {
		       ...
		       rs22_memcpy(&tcp_event->EvtBody.stSocketResp.uSocketType,
		             ((Data_packet) + 2), 2);
			   /* Decode the socket descriptor */
		       rs22_memcpy(&tcp_event->EvtBody.stSocketResp.uSocketDescriptor,
		 	            ((Data_packet) + 4),2);
		      /* Decode the local port number */
		       rs22_memcpy(&tcp_event->EvtBody.stSocketResp.stLocalAddress.sin_port,
			            ((Data_packet)+6), 2);
			  /* Decode the local ip address  */
		      rs22_memcpy(tcp_event->EvtBody.stSocketResp.stLocalAddress.sin_addr.s_addr, 
		                ((Data_packet)+8),4);
		
        	   ii = CPU_TO_LE16(tcp_event->EvtBody.stSocketResp.uSocketDescriptor);
			   uSocketDescriptor = ii;
			  /* Get the free entry in the socket data base */
        	   if((tab_indx = get_free_table_entry()) >= 0)
        	   {
        	     app_indx = identify_table_entry(CPU_TO_LE16(tcp_event->EvtBody.stSocketResp.stLocalAddress.sin_port),
        		 ...
        		 if (app_indx >= 0)
        		 {
        		   ...
        		   gsock_table[tab_indx].prototype = CPU_TO_LE16(tcp_event->EvtBody.stSocketResp.uSocketType);
				   /* Socket create event callback function */
        		   stapid[app_indx].stCb.cb_sock_create_event(ii,
															 CPU_TO_LE16(tcp_event->EvtBody.stSocketResp.stLocalAddress.sin_port));
				   Adaptor.datasend = TRUE;
        		 }
        	   }
			   Adaptor.InterruptStatus = Adaptor.InterruptStatus & (~(SD_DATA_PENDING));
	      	 }
			 LED7 = LED_OFF;
        	...
        	case E_TCP_EVT_TCPConfigEvent:   /* Ipconfig event */
        	{
			  ...
			  index += 4;
			  if((ipConfig->uIPaddr[0] == 0) && (ipConfig->uIPaddr[1] == 0) && \
			  	 (ipConfig->uIPaddr[2] == 0) && (ipConfig->uIPaddr[3] == 0))
			  {
			...
			case E_TCP_EVT_Close:    /* Socket close event */
        	{
              ...
              {
                if (gsock_table[cnt].sockd == tcp_event->EvtBody.stClose.uSocketDescriptor)
                {
              ...
              stapid[Adaptor.apid].stCb.cb_close_event(tcp_event->EvtBody.stClose.uSocketDescriptor);
        	}
        	return RS22_STATUS_SUCCESS;
           	...
           	case E_TCP_EVT_ConnectEvent: /* Got the TCP client connect event */
        	{
			  ...
			  for(cnt = 0; cnt < MAX_TABLE_ENTRY; cnt++)
              {
				if ((gsock_table[cnt].sockd == tcp_event->EvtBody.stConnect.uSocketDescriptor))
				{
				  if(tcp_event->EvtBody.stConnect.uSocketDescriptor == 0)
                  {                  
			                                      ...
			                                      application RECV some data packet*/
        	{
				uint16_t	uDataOffset;
  				uint16_t prototype;
			   /* Decode the socket Descriptor */
               rs22_memcpy(&tcp_event->EvtBody.stRecv.uSocketDescriptor,((Data_packet) + 2),2);
			   /* decode the data paket length */
	           rs22_memcpy(&tcp_event->EvtBody.stRecv.uBufferLength,((Data_packet)+4), 4);
	           rs22_memcpy(&tcp_event->EvtBody.stRecv.uDataOffset,  
		             ((Data_packet)+8),2);
        	  ...
        	  for(cnt =0 ;cnt < MAX_TABLE_ENTRY ; cnt++)
        	  {
        		...
        		if (gsock_table[cnt].sockd == tcp_event->EvtBody.stRecv.uSocketDescriptor)
        		{
        		  Adaptor.apid = gsock_table[cnt].apid;
        		  break;
        		}
        	  ...
        	  stapid[Adaptor.apid].stCb.cb_recv_event(tcp_event->EvtBody.stRecv.buff,
        	...
        	case E_TCP_EVT_RemoteTerminateEvent:
        	{
        	  ...
        	  for(cnt = 0; cnt < MAX_TABLE_ENTRY; cnt++)
        	  {
        	    ...
        	    if (gsock_table[cnt].sockd == tcp_event->EvtBody.stRemoteTerminate.uSocketDescriptor)
        	    {
        	      Adaptor.apid = gsock_table[cnt].apid;
				  gsock_table[cnt].sockd = -1;
				  gsock_table[cnt].sock_status = 0;
        	  ...
        	  stapid[Adaptor.apid].stCb.cb_remoteclose_event(tcp_event->EvtBody.stRemoteTerminate.uSocketDescriptor);
        	  rsi_socket_deregister(Adaptor.apid);
			  while(Delay++ < 20000);
			  /* Initialize socket creation info */
        ...
        switch (type)
        {
        	...
        	case MGMT_DESC_TYP_CARD_RDY_IND: /* Card ready indication firmware loaded */
        	{
        		...
        		if(Adaptor.image_upgrade == 1)
        		{
    			  ...
    			  displayInfo(IMAGE_UPGRADE_COMPLETED); 
				  LED7 = LED_ON;
        	...
        	case MGMT_DESC_INIT_RESPONSE: /* WLAN init Response  */
        	{
        		...
        		rsi_send_scan_request(Adaptor.scan_params.uChannelNo, Adaptor.scan_params.uSSID);
            ...
            break;
             ...
             /* Decode the APs list */
			  ...
			  if(auto_connect_open_ap == 0)
			  {
			    ...
			    {
				  if((rs22_strcmp((const char *)Adaptor.ScanRsp.stScanInfo[index].uSSID, (const char *)Adaptor.join_params.uSSID)) == 0)
				  {
					ssid_found = 1;         //kiran
					break;	  
				  }
        	...
        	case MGMT_DESC_JOIN_RESPONSE:   /* Join response */
        	{
        		    ...
        		    {
						Status = desc[2];
						if((auto_connect_open_ap == 1) && (re_connect < Adaptor.ScanRsp.uScanCount))
						{
						  rsi_rejoin();
						  if (re_connect >= Adaptor.ScanRsp.uScanCount)
						  	return RS22_STATUS_FAILURE;
						}
						else if ((auto_connect_open_ap == 1) && (re_connect >= Adaptor.ScanRsp.uScanCount))
						{
							return Status;
						}
						else 
						{
							if (((auto_connect_open_ap == 0)) && (ssid_found == 1) && (join_retry < 10))
							{                                                             
								join_retry++;
								rsi_rejoin ();
							}
...
)
{
  ...
  do
  {
    ...
    Adaptor.InterruptStatus = ((uint8_t )InterruptStatus & (~InterruptMaskReg));
    ...
    do
    {
      ...
      switch(InterruptType)
      {
        case BUFFER_FULL:
        {
          ...
          Adaptor.once_full++;
		  Adaptor.datasend = FALSE;
          Adaptor.InterruptStatus = Adaptor.InterruptStatus & (~(SD_PKT_BUFF_FULL));
          *size = 0;
        ...
        case BUFFER_FREE:
        {
    	  ...
    	  Adaptor.once_empty++;
    	  *size = 0;
          Adaptor.InterruptStatus = Adaptor.InterruptStatus & (~(SD_PKT_BUFF_EMPTY));
      ...
      {
    	  *size = 0;
    	  Adaptor.InterruptStatus = Adaptor.InterruptStatus & (~(SD_SLEEP_INDCN));
      }
      break;
      case WAKEUP_INDCN:
      {
    	  *size = 0;
    	  ...
    	  Adaptor.InterruptStatus = Adaptor.InterruptStatus & (~(SD_WAKEUP_INDCN));
      }
      break;
      ...
      case MGMT_PENDING:
      {
    	  ...
    	  *size = 0;
    	  Adaptor.InterruptStatus = Adaptor.InterruptStatus & (~(SD_MGMT_PENDING));
      }
      break;

      case DATA_PENDING:
      {
          if ((status = rsi_rcv_pkt(&DataRcvPacket, &pktLen)) != 0)
          {
...
*END****************************************************************************/
uint32_t rsi_socket_register(uint32_t *appId, 
                              uint16_t uPort,
							  uint32_t uIP,
...
int32_t sock_cb0(int32_t sockd,
                uint16_t sourcePort)
{
...
int32_t sock_cb1( int32_t sockd,
                  uint16_t sourcePort )
{
...
*END****************************************************************************/
uint32_t recv_cb0(uint8_t *data,
			    uint32_t BytesReceived, uint16_t sockHandle)
{
  ...
  if( data[0] == LED ) 
  {
   ...
   if(list_sent == 0)
   {
     ...
     if(len > 1)
     {
       while((rsi_send_data(SockHandle0, len, data)) != RSI_STATUS_SUCCESS);
     }
     list_sent++;
   }  
   return(0);
...
uint32_t recv_cb1(uint8_t *buff,
			    uint32_t BytesReceived, uint16_t sockHandle)
{
	 ...
	 switch(Adaptor.socket_created)
	 {
	   ...
	   case 0:  /* First socket */
	   {
        ...
        /* Regester the CallBacks for Socket 0 */
         ...
         rsi_socket_register(&appid0,  networkConfig->stLocalPort,*((uint32_t *)&Adaptor.ipConfig.uIPaddr[0]),&stcbs0);
		 /* Send the socket create request */      
         Status = rsi_socket_create((SOCKET_TYPE)networkConfig->uSocketType, 
					                    networkConfig->stLocalPort,
		                                &networkConfig->remote_address);
		 if(Status == RS22_STATUS_SUCCESS)
		 {
		   Adaptor.socket_created++;
		 }
	   ...
	   case 1: /* Second socket */
	   {
			 ...
			 rsi_socket_register(&appid1, networkConfig->stLocalPort, *((uint32_t *)&Adaptor.ipConfig.uIPaddr[0]),&stcbs1);
...
int16_t rsi_close_user_socket(networkConfig_t *networkConfig)
{
  ...
  {
    if ((stapid[sock_entry].port == networkConfig->stLocalPort) && (stapid[sock_entry].apid != -1))
    {
	  appId  = sock_entry;
      break;
    }
...

{
	...
	{   /* Decode the channel number */
		...
		Adaptor.ScanRsp.stScanInfo[index1].uChannelNumber = scan_resp[scan_index++];
...
*END****************************************************************************/
int16_t rsi_send_data( uint32_t sockDesc, 
					    uint32_t len, 
					    uint8_t *data )
{
   uint32_t 		InterruptStatus;
  ...
  do
  {
   ...
   if(!(InterruptStatus & 0x01) && !(((ps_mode == 1) && (is_pwr_save_enable == FALSE))))
   {	
