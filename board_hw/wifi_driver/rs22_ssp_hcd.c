...
int32_t rs22_ssp_receive_start_token( void )
{
  ...
  while (1)
  {
	...
	SPI_MasterTransceiveByte(&src_buf[0], &dst_buf[0], 1);
...
Description    : This is the wrapper function that calls SPI_MasterTransceiveByte
...
*END****************************************************************************/
void rs22_ssp_send_and_receive(uint8_t *src_ptr, uint8_t *dst_ptr, uint32_t count)
{
  SPI_MasterTransceiveByte(src_ptr, dst_ptr, count);
}
...
{
   SPI_MasterTransceiveByte(NULL, dst_ptr,count);
}
...
{
	SPI_MasterTransceiveByte(src_ptr, NULL, count);
