...
#endif
int32_t     	    rsi_card_read(uint8_t  *frame_desc,uint8_t  ***ptr,uint32_t *pktLen,uint32_t *queueno);
...
int32_t             rsi_socket_create(SOCKET_TYPE uSocketType,
						               uint16_t stLocalPort,
						               stRemoteAdress_t *stRemoteAddress);
uint16_t 			rsi_socket_close (uint16_t uSocketDescriptor);
uint16_t 			rsi_socket_send(uint16_t uSocketDescriptor, uint32_t uBufferLength, uint8_t *pBuffer);
