...
uint32_t accept_cb(int32_t sd, int32_t nsd, uint16_t destinationPort, uint8_t *destinationIpAddress);

uint32_t close_cb(int32_t socketId);

uint32_t recv_cb0(uint8_t *buff, uint32_t BytesReceived, 
                    		 uint16_t sockHandle);
							 
uint32_t recv_cb1(uint8_t *buff, uint32_t BytesReceived, 
                    		 uint16_t sockHandle);
							 
uint32_t remote_close_cb(int32_t socketId);

uint32_t rsi_socket_register(uint32_t *appId, uint16_t uPort,uint32_t uIP,Callbacks_P pCallbacks);
