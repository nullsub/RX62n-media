...
typedef uint32_t (*CB_Sock_Create_Event)(int32_t socketId,
									   uint16_t sourcePort);

typedef uint32_t (*CB_Accept_Event)(int32_t socketId,
								  int32_t newSocketId,
								  uint16_t destinationPort,
								  uint32_t DestinationIpAddress);

typedef uint32_t (*CB_Recv_Event)(uint8_t *pBuf,
							    uint32_t BytesReceived,
								 uint16_t sockHandle);


typedef uint32_t (*User_Recv_Fun)(uint8_t *pBuf,
							    uint32_t BytesReceived,
								 uint16_t sockHandle);
